...
def custom_yolo3_spp_body(inputs, num_anchors, num_classes, weights_path):
    """Create a custom YOLO_v3 SPP model for custom classes."""
    num_classes_coco = 80
    ...
    y1 = DarknetConv2D(
        num_anchors * (num_classes + 5), (1, 1), name='predict_conv_1'
    )(y1)
    y2 = DarknetConv2D(
        num_anchors * (num_classes + 5), (1, 1), name='predict_conv_2'
    )(y2)
    y3 = DarknetConv2D(
        num_anchors * (num_classes + 5), (1, 1), name='predict_conv_3'
    )(y3)
    return Model(inputs, [y1, y2, y3])
...
def tiny_yolo3_body(inputs, num_anchors, num_classes):
    """
    Create Tiny YOLOv3 model CNN body in Keras.

    ▶ float/tiny-yolov3.h5 를 만들 때 사용된 구조와 동일한 형태로 구성.
    """
    # x1: 26x26 feature map (416x416 입력 기준)
    x1 = compose(
...
def tiny_yolo3lite_body(inputs, num_anchors, num_classes):
    """Create Tiny YOLOv3 Lite model CNN body in Keras."""
    f2 = compose(
